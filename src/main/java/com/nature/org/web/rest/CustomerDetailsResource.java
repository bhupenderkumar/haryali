...
import com.nature.org.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link com.nature.org.domain.CustomerDetails}.
 */
@RestController
@RequestMapping("/api")
public class CustomerDetailsResource {
    ...
    public ResponseEntity<List<CustomerDetails>> searchCustomerDetails(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of CustomerDetails for query {}", query);
        Page<CustomerDetails> page = customerDetailsService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
